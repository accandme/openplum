package ch.epfl.data.zad.milestone2.app;
...
public class Query3 extends AbstractApp {
	...
	public void run(String[] args) throws SQLException, InterruptedException {
		...
		dbManager.execute("CREATE TABLE temp_col(l_orderkey INTEGER, revenue FLOAT, o_orderdate DATE, o_shippriority INTEGER)", "node0");
		dbManager.execute("CREATE INDEX ON temp_col(l_orderkey, o_orderdate, o_shippriority)", "node0");
		dbManager.execute(
				"SELECT l_orderkey, " +
