 ...
 * the StepExecutor
 * The generateTmpTblName method is called
 * whenever a component (such as the GraphProcessor 
 * or the SuperDuper) needs to allocate a new
 * unique temporary table name 
 * The cleanTempTables method should be called 
 * at the end in order to clean all the temporarily
 * created tables in the various underlying databases 
 *  
...
public class TableManager {
	 ...
	 * in the given list of nodes, according to the saved
	...
	/**
	 * Cleans all Temporary Tables on the nodes mentioned 
	 * in the given list of nodes
	 * Cleans all tables whose names start with tmp_
	 * Careful: should not be called if another user 
	 * is using the database (running their distributed 
	 * queries) because it might drop tables that
	 * are currently used by them
	 * 
	 * @param List<String> nodeIds
	 * @throws SQLException
	 * @throws InterruptedException
	 ...
	 * large after several SuperDuper steps 
	 * We make sure the name is unique
	 * and keep track of it, so that we
	 * can cleanup later
	 * 
